...
namespace CheckSumTool
{
    ...
    {
        private readonly static ulong[] CrcTable = new ulong[]
        {
        ...
        public static byte[] GetStreamCRC64(Stream stream)
        {
            return GetStreamCRC64(stream, new Crc64ISO());
        }

        public static byte[] GetFileCRC64(string path)
        {
            return GetFileCRC64(path, new Crc64ISO());
        }
        ...
        /// </param>
        public override void Update(ulong bval)
        {
        ...
        public override void Update(byte[] buf, int off, int len)
        {
            ...
            while ( --len >= 0 )
            {
                crc = CrcTable[(byte)((crc ^ buf[off++]) & 0xFF)] ^ (crc >> 8);
            }
        }
    }
}
